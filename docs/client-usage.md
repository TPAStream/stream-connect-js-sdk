...
| `realTimeVerification`        | For realtime validation of logins. If disabled all creds will be assumed correct by the sdk. Defaulted to `true` since 0.8.0 (was undefined / treated as `false` at runtime in 0.7.x).            | Boolean | `realTimeVerification: true`                          | `true`    |
